...
export default class ClapprNerdStats extends UICorePlugin {
  ...
  get playerWidth() {
    return this.core.playerInfo.computedSize.width;
  }
  get playerHeight() {
    return this.core.playerInfo.computedSize.height;
  }

  constructor(core) {
    super(core);
    this._shortcut = get(core, "options.clapprNerdStats.shortcut", [
      "command+shift+s",
      "ctrl+shift+s",
    ]);
    this._iconPosition = get(
      core,
      "options.clapprNerdStats.iconPosition",
      "top-right"
    );
    this.metrics = {};
  }
  ...
  init() {
    ...
    if (typeof clapprStats === "undefined") {
      ...
      Mediator.on(
        `${this.core.options.playerId}:${Events.PLAYER_RESIZE}`,
        this.onPlayerResize,
        this
      );
